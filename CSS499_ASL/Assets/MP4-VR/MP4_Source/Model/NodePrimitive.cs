...
public class NodePrimitive: MonoBehaviour {
    ...
    // reference to collider for this primitive
	...
	public void LoadShaderMatrix(ref Matrix4x4 nodeMatrix)
    {
        ...
        // update primitive's collider based on transformation matrix
        UpdatePrimitiveCollider(m);
    }

    private void UpdatePrimitiveCollider(Matrix4x4 m)
    {
        primCollider.transform.position = m.GetColumn(3);
        primCollider.transform.rotation = m.rotation;
        primCollider.transform.localScale = m.lossyScale;
    }
}